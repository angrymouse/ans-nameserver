...
const { constants, encoding, Hosts, ResolvConf, Hints, util } = require('bns-plus');
...
if (type === 'OPENPGPKEY' && name.indexOf('@') !== -1) {
    const { openpgpkey } = require('bns-plus');
    try {
...
async function resolve(name, type, options) {
    ...
    if (unbound) {
        let { udns } = require('bns-plus');
        dns = udns
    }
    else if (recursive) {
        let { rdns } = require('bns-plus');
        dns = rdns
    }
    else {
        let { dns: realDns } = require('bns-plus');
        dns = realDns
    }
